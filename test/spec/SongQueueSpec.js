describe('SongQueue', function() {
  ...
  describe('when a song is dequeued', function() {
    it('removes the correct song', function() { //Changed this test to confirm whether or not dequeue is dequeuing the correct song versus just removing any song.
      removeSpy = sinon.spy(SongQueue.prototype, 'remove');
      var songQueue = new SongQueue([songData1, songData2]); //Added second song
      song2 = songQueue.at(1);
      songQueue.at(0).dequeue(); //Dequeued first song, now second song should be first in queue.
      expect(removeSpy).to.have.been.called;
      SongQueue.prototype.remove.restore();
      expect(songQueue.at(0)).to.equal(song2); //Checks to make sure first song in queue is now second song.
    });
  });
